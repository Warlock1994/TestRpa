    ...
    <>
      ...
      <div className="space-y-2">
        ...
        <NumberInput
          ...
          max={10000}
        />
        <p className="text-xs text-muted-foreground">鼠标悬停在元素上的时长，单位毫秒</p>
      </div>
    </>
  )
}
  ...
  return (
    <>
      ...
      {(data.waitType as string) === 'time' || !data.waitType ? (
        <div className="space-y-2">
          <Label htmlFor="duration">等待时长(毫秒)</Label>
          <VariableInput
...
export function PlaySoundConfig({ data, onChange }: { data: NodeData; onChange: (key: string, value: unknown) => void }) {
  ...
  return (
    <>
      ...
      <div className="space-y-2">
        <Label htmlFor="beepInterval">'提示音间隔(毫秒)'</Label>
        <NumberInput
          id="beepInterval"
...
export function ViewImageConfig({ data, onChange }: { data: NodeData; onChange: (key: string, value: unknown) => void }) {
  ...
  return (
    <>
      ...
      {data.autoClose && (
        <div className="space-y-2">
          <Label htmlFor="displayTime">'显示时长(毫秒)'</Label>
          <NumberInput
            id="displayTime"
            value={(data.displayTime as number) ?? 5000}
